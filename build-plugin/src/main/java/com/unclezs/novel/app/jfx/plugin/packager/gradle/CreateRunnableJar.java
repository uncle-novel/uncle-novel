...
public class CreateRunnableJar extends ArtifactGenerator {
  ...
  protected File doApply(Packager packager) {
    ...
    if (libsFolder != null && libsFolder.exists()) {
      dependencies = Arrays.stream(Objects.requireNonNull(libsFolder.listFiles()))
          .map(f -> libsFolder.getName() + "/" + f.getName()).collect(Collectors.toList());
    }
    ...
    jarTask.getManifest().getAttributes().put("Created-By",
        "Gradle " + Context.getGradleContext().getProject().getGradle().getGradleVersion());
    ...
    jarTask.getManifest().getAttributes()
        .put("Class-Path", StrUtil.join(" ", dependencies.toArray(new Object[0])));
    ...
    if (manifest != null) {
      ...
      manifest.getSections()
          .forEach(s -> jarTask.getManifest().attributes(s.getEntries(), s.getName()));
    }
