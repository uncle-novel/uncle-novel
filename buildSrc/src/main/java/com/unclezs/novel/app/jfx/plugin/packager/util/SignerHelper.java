    ...
    private AuthenticodeSigner build() throws SignerException {
        ...
        if ("PKCS11".equals(storetype)) {
            ...
            if (keystore != null && keystore.exists()) {
                ...
                throw new SignerException("keystore should either refer to the SunPKCS11 configuration file or to the name of the provider configured in jre/lib/security/java.security");
            }
        }
        ...
        if (keystore != null) {
            ...
            if (alias == null) {
                if (aliases.size() == 1) {
                    ...
                    throw new SignerException("alias must be set to select a certificate (available aliases: " + String.join(", ", aliases) + ")");
                }
            }
            ...
            if (certfile != null) {
                if (chain.length != 1) {
                    throw new SignerException("certfile can only be specified if the certificate from the keystore contains only one entry");
                }
