      ...
      project.getConvention().getPlugin(JavaPluginConvention.class).getSourceSets()
          .all(sourceSet -> {
            ...
            for (File srcDir : srcDirs) {
              SassCompile sassCompile = project.getTasks()
                  .create(i == 1 ? taskName : taskName + i, SassCompile.class);
              i++;
              sassCompile.setGroup(BasePlugin.BUILD_GROUP);
              sassCompile.setDescription(
                  "Compile sass and scss files for the " + sourceSet.getName() + " source set");
              Copy processResources = (Copy) project.getTasks()
                  .getByName(sourceSet.getProcessResourcesTaskName());
              sassCompile.setSourceDir(srcDir);
              sassCompile.setDestinationDir(
                  jSass.isInplace() ? srcDir : processResources.getDestinationDir());
