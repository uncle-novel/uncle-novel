package com.unclezs.novel.app.jfx.framework.ui.components.icon;

import com.unclezs.novel.app.jfx.framework.util.StrUtils;
...
public class SvgIcon extends StackPane {
  ...
  public static void load(String configPath) {
    try {
      ...
      properties.load(SvgIcon.class.getResourceAsStream(configPath));
      for (String name : properties.stringPropertyNames()) {
        String path = properties.getProperty(name);
        if (StrUtils.isNotBlank(path)) {
          register(name, path);
          log.debug("注册svg图标成功：name={}, 当前svg图标数量：{}", name, SVG_PATHS.size());
        }
      }
    } catch (IOException e) {
      log.error("svg 图标加载失败", e);
    }
  }
  ...
  public ObjectProperty<Paint> fillProperty() {
    if (fill == null) {
      this.fill = new SimpleStyleableObjectProperty<>(StyleableProperties.TEXT_FILL, SvgIcon.this,
        "-fx-text-fill", Color.BLACK) {
        @Override
        protected void invalidated() {
          icon.setBackground(new Background(
            new BackgroundFill(fillProperty().get() == null ? Color.BLACK : getValue(), null,
              null)));
        }
      };
    }
  ...
  public void setSize(double width, double height) {
    ...
    this.icon.setMinSize(width, height);
    this.icon.setMaxSize(width, height);
  }
  ...
  public DoubleProperty sizeProperty() {
    if (this.size == null) {
      size = new SimpleStyleableDoubleProperty(StyleableProperties.SIZE, SvgIcon.this, "size",
  ...
  private static class StyleableProperties {
    ...
    private static final CssMetaData<SvgIcon, Number> SIZE = new CssMetaData<>("-fx-icon-size",
    ...
    private static final CssMetaData<SvgIcon, Paint> TEXT_FILL = new CssMetaData<>("-fx-text-fill",
      PaintConverter.getPaintConverter()) {
      @Override
      public boolean isSettable(SvgIcon control) {
